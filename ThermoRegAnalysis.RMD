...
print(paste("Last update:", Sys.time()))
...
# 10K points is about the max I can do in a reasonable time.
set.seed(17444)
...
AICc(g02$mer) # this AICc is higher than above
...
AICc(g03$mer) # this is lowest

# refit with REML=TRUE
g01 <- gamm4(temp ~ s(ambient, by = treatment, k = 5) + s(time1, by = treatment, k = 5) + tempIncrease + treatment, random = ~ (1|colony) + (1|dayInt), data = airdta_sm, REML = TRUE)
...
s1 <- summary(g02$mer)
# variance for random effects, including smooths
stargazer(as.data.frame(VarCorr(g02$mer), type = 'html'))
write.csv(file = "dataTables/VarRandEffectsAir.csv", as.data.frame(VarCorr(g02$mer)))
# parametric effects
stargazer(s1$coefficients, type = 'html')
...
write.csv(x = all_data, file = paste("dataTables", "combined_tables.csv", sep = "/"))
```

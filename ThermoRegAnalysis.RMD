---
title: "Thermoregulation Analysis"
...
# remove figures in folder (so that I don't get a backlog of old files)
#do.call(file.remove, list(list.files('figures', full.names = TRUE)))


# make a dataTables directory, if it doesn't exist
...
p1 <- ggplot(drop_na(combData[seq(1, nrow(combData), length.out = 10000), ]), aes(x = time, y = ambient)) + 
  geom_line(size = 0.1) + 
...
### this analysis shows that treatment affects both air and brood temperatures. 
### Permutation tests compare mean temperatures between treatment and control groups
### We generated the null distribution
```{R}
...
# create bootstrap CI
bootFunc <- function(treatment, temp){
  # get a different permutation of treatment
  t2 <- sample(1:length(treatment), replace = TRUE)
  diff(tapply(temp[t2], treatment[t2], mean))
}

# conduct permutation test for air
perms <- sapply(1:100000, FUN = function(i) permFunc(mdf$treatment, mdf$airTemp))
bootSamps <- sapply(1:100000, function(i) bootFunc(mdf$treatment, mdf$airTemp))
actualDiff <- diff(tapply(mdf$airTemp, mdf$treatment, mean))

# p-value for randomization test for airTemp
pval = mean(perms <= actualDiff | perms >= - actualDiff) # ~0.00822
c(actualDiff, quantile(as.numeric(bootSamps), probs = c(0.025, 0.975), na.rm = TRUE), Air_pval = pval)
write.csv(x = t(c(actualDiff, quantile(as.numeric(bootSamps), probs = c(0.025, 0.975), na.rm = TRUE), 
                Air_pval = pval)), file = "dataTables/AirPermTest.csv")


# conduct permutation test for brood
bootSamps <- sapply(1:100000, function(i) bootFunc(mdf$treatment, mdf$broodTemp))
...
# p-value for randomization test for airTemp
pval = mean(perms <= actualDiff | perms >= - actualDiff) # ~0.00491
c(actualDiff, quantile(as.numeric(bootSamps), probs = c(0.025, 0.975), na.rm = TRUE), Brood_pval = pval)
write.csv(x = t(c(actualDiff, quantile(as.numeric(bootSamps), probs = c(0.025, 0.975), na.rm = TRUE), 
                Brood_pval = pval)), file = "dataTables/BroodPermTest.csv")
```


```{r}
# here's how you could calculate 95% CI's for each group (for instance, for plotting).


# # calculate 95%CI for each treatment
# meanCIfunc <- function(treatment, temp){
#   t2 <- tapply(temp, INDEX = treatment, sample, replace = TRUE)
#   return(c(mean(t2$c), mean(t2$t)))
# }
# 
# IndividualCI_samps <- sapply(1:1000, function(i) meanCIfunc(mdf$treatment, mdf$airTemp))
# AirCIDF <- apply(as.data.frame(t(IndividualCI_samps)), MARGIN = 2, FUN = quantile, c(0.025, 0.975))
# adf2 <- as.data.frame(rbind(AirCIDF, tapply(mdf$airTemp, INDEX = mdf$treatment, FUN = mean)))
# adf2[4, ] <- c("c", "t")
# t(adf2)
# perms <- sapply(1:100000, FUN = function(i) permFunc(mdf$treatment, mdf$airTemp))
```
...
pdf("figures/smoothplot_brood.pdf", width = 15, height = 15)
par(mfrow = c(3,4))
...
{pdf("figures/smoothplot_brood_labels.pdf", width = 14, height = 14)
par(mfrow = c(3,4))
...
plot.gam(select = 6, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for brood temp, \nTreatment group, Temp decreasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

plot.gam(select = 7, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for brood temp, \nControl group, Temp increasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

plot.gam(select = 8, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for brood temp, \nTreatment group, Temp increasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

par(mai= c(1.5,1,0.3,0))

plot.gam(select = 9, xlabs = "", ylabs = "Partial residuals for brood temp (°C)", g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE, xaxt = "n")
axis(1, labels = FALSE)
...
text(cex=1, x=1:4 - 0.75, y=-3.4, labs, xpd=TRUE, srt=15)
text(cex=1, x=3 - 0.75, y=-3.8, "Treatment group and temp direction", xpd=TRUE, srt=0)
...
pdf('figures/gamSmoothOnIncreasingDecreasing_Brood.pdf', width = 10, height = 7)
...
{pdf("figures/smoothplot_air_labels.pdf", width = 14, height = 14)
par(mfrow = c(3,4))
par(mai= c(0.6,1,0.3,0))
...
plot.gam(select = 5, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for air temp, \nControl group, Temp decreasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

plot.gam(select = 6, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for air temp, \nTreatment group, Temp decreasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

plot.gam(select = 7, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for air temp, \nControl group, Temp increasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

plot.gam(select = 8, xlab = "Time of day (0.5 = noon)", ylab = 'Estimated smooth for air temp, \nTreatment group, Temp increasing (°C)', g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)

par(mai= c(1.5,1,0.3,0))

plot.gam(select = 9, xlabs = "", ylabs = "Partial residuals for air temp (°C)", g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE, xaxt = "n")
axis(1, labels = FALSE)
labs <- gsub(x = levels(brooddta_sm$treatTempIncrInt), pattern = "_grp.", replacement = " ")
labs = gsub("(^|[[:space:]])([[:alpha:]])", "\\1\\U\\2", labs, perl=TRUE) # capitalize
text(cex=1, x=1:4 - 0.75, y=-2.4, labs, xpd=TRUE, srt=15)
text(cex=1, x=3 - 0.75, y=-2.8, "Treatment group and temp direction", xpd=TRUE, srt=0)

par(las = 1)
plot.gam(select = 10, xlabs = "Cohort", ylabs = "Partial residuals for air temp (°C)", g01$gam, all.terms = TRUE, rug = FALSE, shade = TRUE)
...
pdf('figures/gamSmoothOnIncreasingDecreasing_AirAndBrood.pdf', width = 10, height = 7)
